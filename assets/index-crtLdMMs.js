(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const l of i)if(l.type==="childList")for(const k of l.addedNodes)k.tagName==="LINK"&&k.rel==="modulepreload"&&s(k)}).observe(document,{childList:!0,subtree:!0});function a(i){const l={};return i.integrity&&(l.integrity=i.integrity),i.referrerPolicy&&(l.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?l.credentials="include":i.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function s(i){if(i.ep)return;i.ep=!0;const l=a(i);fetch(i.href,l)}})();const h=document.querySelector("#app");let o="home",L="home",m="Mind",v=!1,T=0;const c=[{id:1,name:"Nagy Petra",className:"12.D",sold:16,issued:20,collected:100800},{id:2,name:"Kiss Marcell",className:"12.B",sold:14,issued:20,collected:75600},{id:3,name:"Tóth Anna",className:"12.A",sold:11,issued:18,collected:56700}],z=[{id:1,school:"Kölcsey Ferenc Gimnázium",contact:"Varga Márk · 12.B",relation:"a saját kódoddal szerződött",code:"BALINT-12D",status:"Szerződött",commission:25e3},{id:2,school:"Szent László Gimnázium",contact:"Kovács Anna · 12.A",relation:"az ajánlói linkedről érkezett",code:"BALINT-12D",status:"Egyeztetés",commission:25e3},{id:3,school:"Veres Pálné Gimnázium",contact:"Szabó Bence · 12.D",relation:"a saját kódoddal regisztrált",code:"BALINT-12D",status:"Bemutatva",commission:25e3}],d=["home","tickets","promoters","affiliates","finance"],r=()=>c.reduce((t,e)=>t+e.sold,0),y=()=>c.reduce((t,e)=>t+e.issued,0),S=()=>c.reduce((t,e)=>t+e.collected,0),n=t=>`${t.toLocaleString("hu-HU")} Ft`;function A(){h.innerHTML=`
  ...
  `}function w(){return`
    <div class="content home-screen">
      ...
      <div class="stats-grid">
        ...
        <article><span>JÓVÁÍRT JUTALÉK</span><strong>${n(z.filter(t=>t.status==="Szerződött").reduce((t,e)=>t+e.commission,0))}</strong><small>1 sikeres szerződés</small></article>
      </div>
  ...
  `}function M(){const t=r()*6300;return`
  ...
  `}function O(){const t=c.filter(e=>m==="Mind"||(m==="Elszámolt"?e.collected>=e.sold*6300:e.collected<e.sold*6300));return`
  ...
  `}function R(){return`
    <div class="content affiliates-screen">
      ...
      <div class="affiliate-list">
        ${z.map(t=>`<article><span><b>${t.school}</b><small>${t.contact}<br />${t.relation}</small></span><div><em class="status status-${j(t.status)}">${t.status.toUpperCase()}</em><strong>${t.status==="Szerződött"?n(t.commission):`várható ${n(t.commission)}`}</strong></div></article>`).join("")}
      </div>
      <button class="primary full-width add-button" data-action="add-affiliate">+ ÚJ ISKOLA AJÁNLÁSA</button>
    </div>
  `}function I(){const t=z.filter(s=>s.status==="Szerződött").reduce((s,i)=>s+i.commission,0),e=12e3,a=t+e;return`
